def deep_map(f, s):
    ...
    for i in range(len(s)):
        if not (type(s[i]) == list):
            ...
            deep_map(f, s[i])
    '''
    This is a wrong implementation because when it executes s = f(s), it doesn't change the original list 
...
def balanced(m):
    ...
    left_weight, right_weight = length(left_arm) * total_mass(end(left_arm)), length(right_arm) * total_mass(end(right_arm))
