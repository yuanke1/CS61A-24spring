...
def buy(required_fruits, prices, total_amount):
    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            fruit = fruits[0]
            price = [p * prices[fruit] for p in range(1, amount//prices[fruit]+1)]
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    x = make_city('center', lat, lon)
    return get_name(city_a) if distance(x, city_a) <= distance(x, city_b) else get_name(city_b)
